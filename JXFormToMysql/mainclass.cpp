...
void mainClass::setParameters(int p_argc, char *p_argv[])
{
    argc = p_argc;
    argv = *p_argv;
}
...
void mainClass::isFieldValid(QString field, bool select)
{
...
void mainClass::loadInvalidFieldNames()
{
...
void mainClass::addRequiredFile(QString fileName)
{
    bool found = false;
...
void mainClass::checkFieldName(TtableDef table, QString fieldName)
{
...
void mainClass::checkTableName(QString tableName)
{
...
int mainClass::isSelect(QString variableType)
{
    QString varType;
...
bool mainClass::isNote(QString variableType)
{
    QString varType;
    varType = variableType.toLower().trimmed();
...
bool mainClass::isCalculate(QString variableType)
{
    QString varType;
    varType = variableType.toLower().trimmed();
...
void mainClass::report_file_error(QString file_name)
{
    QDomElement XMLRoot;
...
QString mainClass::fixColumnName(QString column)
{
    QString res;
    res = column;
...
bool mainClass::isColumnValid(QString column)
{
...
int mainClass::convertCSVToSQLite(QString fileName, QDir tempDirectory, QSqlDatabase database)
{
...
QDomElement mainClass::getTableCreateElement(QString table)
{
    QDomElement res;
...
QDomElement mainClass::getTableElement(QString table)
{
    QDomElement res;
    for (int pos = 0; pos <= tables.count()-1; pos++)
    {
...
QString mainClass::getDescForLanguage(QList<TlngLkpDesc > lkpList, QString langCode)
{
...
QString mainClass::getDefLanguage()
{
...
QString mainClass::getDefLanguageCode()
{
    for (int pos = 0; pos <= languages.count()-1; pos++)
    {
        if (languages[pos].deflang == true)
...
QString mainClass::getLanguageCode(QString languageName)
{
    for (int pos = 0; pos <= languages.count()-1; pos++)
    {
...
int mainClass::getMaxDescLength(QList<TlkpValue> values, int minimum)
{
    hasSelects = true;
    int res;
    res = 0;
...
int mainClass::getMaxValueLength(QList<TlkpValue> values, QString fieldType, int minimum)
{
    int res;
    res = 0;
    int pos;
...
bool mainClass::areValuesStrings(QList<TlkpValue> values)
{
    bool ok;
    int pos;
    for (pos = 0; pos <= values.count()-1;pos++)
...
QString mainClass::get_related_usage(QString table)
{
    for (int pos = 0; pos < tables.count(); pos++)
    {
...
TtableDef mainClass::checkDuplicatedLkpTable(QString table, QList<TlkpValue> thisValues, bool select_from_file, QString lookupCSV)
{
    TtableDef empty;
    empty.name = "EMPTY";
    empty.isLoop = false;
...
QString mainClass::getKeyField(QString table)
{
    int pos2;
    QString res;
    for (int pos = 0; pos <= tables.count()-1; pos++)
...
QString mainClass::getRelatedName(TtableDef table, QString relatedTable)
{
    int pos;
...
// that are related to a table
QString mainClass::getForeignColumns(TtableDef table, QString relatedTable)
{
    QString res;
    int pos;
    for (pos = 0; pos <= table.fields.count()-1; pos++)
...
// that are related to a table
QString mainClass::getReferencedColumns(TtableDef table, QString relatedTable)
{
    QString res;
    int pos;
    for (pos = 0; pos <= table.fields.count()-1; pos++)
...
bool mainClass::isRelatedTableLookUp(QString relatedTable)
{
    int pos;
...
void mainClass::generateOutputFiles(QString ddlFile,QString insFile, QString metaFile, QString xmlFile, QString transFile, QString XMLCreate, QString insertXML, QString dropSQL)
{
    QStringList fields;
    QStringList indexes;
    QStringList keys;
...
TfieldMap mainClass::mapODKFieldTypeToMySQL(QString ODKFieldType)
{
    TfieldMap result;
    result.type = "text";
    result.size = 255;
...
int mainClass::getTableIndex(QString name)
{
    for (int pos = 0; pos <= tables.count()-1;pos++)
    {
        if (!tables[pos].islookup)
...
bool mainClass::isDefaultLanguage(QString language)
{
...
int mainClass::getCodedLangIndexByName(QString language)
{
    for (int pos=0; pos < languages.count();pos++)
    {
        if (languages[pos].desc.toLower().trimmed() == language.toLower().trimmed())
...
int mainClass::genLangIndexByName(QString language)
{
    for (int pos=0; pos < languages.count();pos++)
    {
        if (languages[pos].desc.toLower().trimmed() == language.toLower().trimmed())
...
void mainClass::addToStack(QString groupOrRepeat, QString type)
{
    variableStack.append(groupOrRepeat + "@" + type);
}

//Fixes table and field name by removing invalid MySQL characters
QString mainClass::fixField(QString source, bool select)
{
    QString res;
...
QString mainClass::getVariableStack(bool full)
{
    QString res;
...
QString mainClass::getTopRepeat()
{
    if (repeatStack.count() > 0)
...
TtableDef mainClass::getTable(QString name)
{
    TtableDef res;
    res.name = "";
    res.isLoop = false;
...
bool mainClass::selectHasOrOther(QString variableType)
{
...
QList <TlngLkpDesc > mainClass::getLabels(QJsonValue labelValue)
{
...
bool mainClass::checkSelectValue(QString variableName, QList<TlkpValue> values, QString value, bool report, bool multiSelect)
{
    if (multiSelect)
    {
        if (value.indexOf(" ") >= 0)
...
QList<TlkpValue> mainClass::getSelectValuesFromGeoJSON(QString variableName, QString variableType, QString fileName, int &result, QDir dir, QString codeColumn, QString descColumn, QStringList &propertyList, QStringList &propertyTypes)
{
...
// This return the values of a select that uses an external xml file.
// e.g., "select one from file a_file.xml" and "select multiple from file a_file.xml"
QList<TlkpValue> mainClass::getSelectValuesFromXML(QString variableName, QString variableType, QString fileName, bool hasOrOther, int &result, QDir dir, QString codeColumn, QString descColumn)
{
    bool multiSelect = false;
    if (isSelect(variableType) == 3)
        multiSelect = true;
...
QList<TlkpValue> mainClass::getSelectValuesFromCSV2(QString variableName, QString variableType, QString fileName, bool hasOrOther, int &result, QDir dir, QSqlDatabase database, QString queryValue, QString codeColumn, QString descColumn)
{
    bool multiSelect = false;
    if (isSelect(variableType) == 3)
    {
...
QList<TlkpValue> mainClass::getSelectValuesFromCSV(QString searchExpresion, QJsonArray choices,QString variableName, QString variableType, bool hasOrOther, int &result, QDir dir, QSqlDatabase database, QString &file, QString &codeColumn, QString &descColumn)
{
    bool multiSelect = false;
    if (isSelect(variableType) == 3)
        multiSelect = true;
...
QStringList mainClass::getExtraColumns(QJsonArray choices)
{
    QStringList res;
    for (int nrow = 0; nrow < choices.count(); nrow++)
    {
...
//This return the list of extra columns as a StrinList
QStringList mainClass::getExtraColumnsTypes(QJsonArray choices, QStringList extra_colums)
{
    QStringList res;
...
bool mainClass::checkColumnName(QString name)
{
...
QList<TlkpValue> mainClass::getSelectValues(QString variableName,QString variableType,QJsonArray choices, bool hasOther, QStringList extraColumns)
{
    bool multiSelect = false;
    if (isSelect(variableType) == 3)
        multiSelect = true;
...
void mainClass::getReferenceForSelectAt(QString calculateExpresion,QString &fieldType, int &fieldSize, int &fieldDecSize, QString &fieldRTable, QString &fieldRField)
{
    int pos;
    //Extract the file from the expression
...
QString mainClass::getUUIDCode(bool last)
{
    QUuid triggerUUID=QUuid::createUuid();
    QString strTriggerUUID=triggerUUID.toString().replace("{","").replace("}","").replace("-","_");
    if (last)
...
void mainClass::parseOSMField(TtableDef &OSMTable, QJsonObject fieldObject)
{
...
void mainClass::parseField(QJsonObject fieldObject, QString mainTable, QString mainField, QDir dir, QSqlDatabase database, QString varXMLCode)
{
...
void mainClass::getJSONRootVariables(QJsonObject JSONObject, QList <TsurveyVariableDef> &surveyVariables)
{
...
void mainClass::parseTable(QJsonObject tableObject, QString tableType, bool repeatOfOne)
{
    QString tableName;
    tableName = getTopRepeat();
...
bool mainClass::isRepeatOfOne(QString repeat_control)
{
...
void mainClass::parseJSONObject(QJsonObject JSONObject, QString mainTable, QString mainField, QDir dir, QSqlDatabase database)
{
...
void mainClass::getLanguages(QJsonObject JSONObject, QStringList &languageList, int &num_labels)
{
...
void mainClass::reportDuplicatedFields()
{
    QDomElement XMLRoot;
...
void mainClass::reportDuplicatedTables()
{
    QDomElement XMLRoot;
...
void mainClass::reportSelectDuplicates()
{
    QDomElement XMLRoot;
...
void mainClass::reportSelectInvalid()
{
    QDomElement XMLRoot;
...
QList<TlkpValue> mainClass::getValuesFromInsertFile(QString tableName, QDomNode startNode, QString &clmCode, QString &cmlDesc)
{
...
int mainClass::genLangIndex(QString langCode)
{
    for (int pos=0; pos < languages.count();pos++)
    {
...
int mainClass::addLanguage(QString langCode, bool defLang, bool coded)
{
...
int mainClass::addLanguage2(QString code, QString name, bool defLang)
{
...
int mainClass::processJSON(QString inputFile, QString mainTable, QString mainField, QDir dir, QSqlDatabase database)
{
...
int mainClass::getFieldIndex(TtableDef table, QString fieldName)
{
...
void mainClass::protect_sensitive()
{
